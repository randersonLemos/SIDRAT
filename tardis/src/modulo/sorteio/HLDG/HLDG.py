...
class HLDG(SorteioPadrao):
    """
    Classe para realizações dos sorteios para ciração das amostras com Hiper
    Cubo Latino.
    """
    def __init__(self):
    ...
    def _gera_solucoes(self):
        ...
        dict_variaveis = self._solucao_referencia.variaveis.get_variaveis_by_tipo(EnumTipoVariaveis.VARIAVEL)

        amostras = []

        for nome_variavel in dict_variaveis:
            ...
            niveis = dict_variaveis[nome_variavel].dominio.niveis
            probabilidade = dict_variaveis[nome_variavel].dominio.probabilidade

            if round(sum(probabilidade), 3) != round(1, 3):
                self.log(tipo=EnumLogStatus.ERRO_FATAL, texto='Somatório das probabilidades diferente de 1')
        ...
        solucoes_df = pd.DataFrame(amostras, columns=dict_variaveis.keys())

        self._solucoes = Solucoes()
        for i in range(solucoes_df.shape[0]):
            self._ultimo_id += 1
            self._ultimo_id = int(self._ultimo_id)
            self._iteracao  = int(self._iteracao)
