...
class Knapsack(ProblemaFechadoPadrao):
    ...
    def __init__(self):
        ...
        self._necessidade = [EnumAtributo.PATH_RESULTADO] + super(Knapsack, self).necessidade
        """ 
        Contem a lista de todos os atributos necessários para o módulo ser executado.
        """

        self._base_folder_name = f'base_KNAPSACK'
        """
        Variavel com o nome do arquivo
        """

        self._dominio_file_name = 'dominio_knapsack_no_repetition.txt'
        """
    ...
    def run(self, contexto: Contexto):
        ...
        """
        super(Knapsack, self).run(contexto)
        ...
        dominio_file_name = self._dominio_file_name

        Copy.copy_file(os.path.join(folder_data_base, dominio_file_name),
                       os.path.join(new_base_path, dominio_file_name))
    ...
    def _verifica_config(self):
        ...
        if not (self._contexto.tem_atributo(EnumAtributo.INICIALIZACAO_DOMINIO) and self._contexto.get_atributo(
                EnumAtributo.INICIALIZACAO_DOMINIO) == os.path.join(self._base_folder_name,
                                                                    self._dominio_file_name)):

            self._contexto.set_atributo(EnumAtributo.INICIALIZACAO_DOMINIO,
                                        [os.path.join(self._base_folder_name, dominio_file_name)],
                                        sobrescreve=True)
