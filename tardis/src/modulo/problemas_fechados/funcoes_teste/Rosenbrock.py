...
import src.modulo.problemas_fechados.funcoes_teste.base.rosenbrock
...
class Rosenbrock(ProblemaFechadoPadrao):
    def __init__(self):
        ...
        self._necessidade = [EnumAtributo.PATH_RESULTADO] + super(Rosenbrock, self).necessidade
        """
        Contem a lista de todos os atributos necessários para o módulo ser executado.
        """

        self._base_folder_name = f'base_ROSENBROCK'
        """
        Variavel com o nome do arquivo
        """

        self._dominio_file_name = 'dominio_rosenbrock.txt'
        """
    ...
    def run(self, contexto: Contexto):
        ...
        """
        super(Rosenbrock, self).run(contexto)

        new_base_path = os.path.join(self._contexto.get_atributo(EnumAtributo.PATH_PROJETO), self._base_folder_name)

        folder_data_base = os.path.dirname(src.modulo.problemas_fechados.funcoes_teste.base.rosenbrock.__file__)
