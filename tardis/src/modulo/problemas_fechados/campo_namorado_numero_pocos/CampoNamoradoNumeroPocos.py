...
class CampoNamoradoNumeroPocos(ProblemaFechadoPadrao):
    def __init__(self):
        super(CampoNamoradoNumeroPocos, self).__init__()
        ...
        self._necessidade = [EnumAtributo.PATH_RESULTADO] + super(CampoNamoradoNumeroPocos, self).necessidade
        """
        Contem a lista de todos os atributos necessários para o módulo ser executado.
        """

        self._base_folder_name = f'base_CampoNamoradoNumeroPocos'
        """
        Variavel com o nome do arquivo
        """

    def run(self, contexto: Contexto):
        super(CampoNamoradoNumeroPocos, self).run(contexto)
        ...
        if not os.path.exists(new_base_path):
            self.log(texto=f'Criando folder base do modelo caixa preta CampoNamoradoNumeroPocos')
            if not Copy.copy_folder(folder_data_base, new_base_path):
                self.log(tipo=EnumLogStatus.ERRO_FATAL, texto=f'Erro ao gerar pasta base para o problema CampoNamoradoNumeroPocos')
            try:
    ...
    def _verifica_config(self):
        self.log(texto=f'Ajustando configuracao para CampoNamoradoNumeroPocos')
